...
export async function createDashboard(tirCounts, periodLength, clinicId, tagId, creds) {
    let patientIds = [];
    ...
    for (const key in tirCounts) {
        for (let i = 0; i < tirCounts[key]; i++) {
            ...
            let patientId = await Create.createPatient(creds.userName, creds.password, creds.baseUrl, clinicId, payload);
            counter++;
            if (patientId) {
...
export async function createMedtronicDashboard(tirCounts, clinicId, tagId, creds) {
    let patientIds = [];
    console.log('Creating patients');
    let counter = 0;
...
export async function createDashboardOffset(tirCounts, periodLength, offsetTimeMinutes, patientName, clinicId, tagId, creds) {
    let patientIds = [];
    console.log('Creating patients');
    let counter = 0;
    for (const key in tirCounts) {
