import * as deleteClaimedPatient from "./deleteClaimedPatient.js";
 ...
 * @param tagId Tag ID
 */
export async function deletePatients(creds: Credentials, clinicId: string, tagId: string): Promise<void> {
  let ids: string[] = [];
  do {
    ids = [];
    ...
    for (const id of ids) {
      ...
      await deleteClaimedPatient.deleteClaimedPatient(creds, clinicId, id);
    }
