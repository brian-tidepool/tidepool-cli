...
interface tagsList {
clinic:{patientTags:{id:string,name:string}[]};
    
}


export async function getTags<T>(
    creds:Credentials
): Promise<tagsList[]|null> {
    try {
        ...
        const postResponse = await fetch(creds.baseUrl + `/v1/clinicians/${userId}/clinics?limit=1000&offset=0`, {
            method: 'GET',
            headers: {
                'Accept': '*/*',
